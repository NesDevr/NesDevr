### View My Portfolio
https://nestorportf.vercel.app/
&nbsp;

I'm a **Software Engineer** with 5+ years of experience building scalable systems (ERPs, Fintech, SaaS, B2B) and data pipelines. I enjoy turning messy, real-world workflows into reliable, production-grade products that make people's work faster and decisions smarter.

## &nbsp;Professional Highlights

- **Software Engineer** @ radii. *(Current)*
  Building ML-powered pricing systems, event-driven Azure architecture, and workflow orchestration with LangGraph

- **Software Engineer** @ Athena Systems
  Built financial data tools with SQL/Python/Flask that reduced support resolution time by 30%

- **Full Stack Developer** @ Inprosa
  Developed tournament management systems with Laravel/Livewire

- **Web Developer / Data Analyst** @ Fort Group
  Created ERP systems and operational dashboards with Python, SQL, and Power BI
&nbsp;

## &nbsp;Featured Projects

[![Trading Backtester](https://img.shields.io/badge/-Trading_Backtester-2d2d2d?style=flat&logo=python&logoColor=white)](https://github.com/NesDevr/BacktestingStrategies)
Data-intensive app processing 10,000+ market data points for strategy backtesting

[![POS System](https://img.shields.io/badge/-Retail_POS_System-2d2d2d?style=flat&logo=laravel&logoColor=white)](https://github.com/NesDevr/SalesSystemLivewireV3)
Full-featured sales platform with inventory management (Laravel/Livewire/Docker)

[![Discord Bot](https://img.shields.io/badge/-Gaming_Discord_Bot-2d2d2d?style=flat&logo=discord&logoColor=white)](https://github.com/NesDevr/YalaharBot/)
Full-stack gaming assistant with React dashboard + Django API tracking 500+ players

[![Athena Toolkit](https://img.shields.io/badge/-Athena_Internal_Tool-2d2d2d?style=flat&logo=flask&logoColor=white)](#)
Internal Flask app that accelerated financial data resolution (Private)
&nbsp;

## &nbsp;Technical Expertise

### Backend & APIs
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="40" height="40" alt="Python"/> &nbsp;
<img src="https://www.svgrepo.com/show/353657/django-icon.svg" width="40" height="40" alt="Django"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" width="40" height="40" alt="FastAPI"/> &nbsp;
<img src="https://skillicons.dev/icons?i=flask" width="40" height="40" alt="Flask"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/laravel/laravel-original.svg" width="40" height="40" alt="Laravel"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postman/postman-original.svg" width="40" height="40" alt="Postman"/> &nbsp;

### Cloud & DevOps
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="40" height="40" alt="Azure"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-plain-wordmark.svg" width="40" height="40" alt="AWS"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="40" height="40" alt="Docker"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="40" height="40" alt="PostgreSQL"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain-wordmark.svg" width="40" height="40" alt="SQL Server"/> &nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" width="40" height="40" alt="Git"/> &nbsp;

### AI/ML & Data
<img src="https://avatars.githubusercontent.com/u/126733545?s=200&v=4" width="40" height="40" alt="LangChain"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/scikitlearn/scikitlearn-original.svg" width="40" height="40" alt="Scikit-learn"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" width="40" height="40" alt="Pandas"/> &nbsp;

### Frontend
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="40" height="40" alt="React"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" width="40" height="40" alt="Tailwind"/> &nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="40" height="40" alt="JavaScript"/> &nbsp;

